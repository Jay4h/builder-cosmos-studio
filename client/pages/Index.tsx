...
const attritionTrend = Array.from({ length: 12 }, (_, i) => ({
  month: i + 1,
  risk: 6 + Math.sin(i / 2) * 2,
}));
...
function Heatmap() {
  ...
  return (
    <div className="grid grid-cols-7 gap-2">
      <div />
      {units.map((u) => (
        <div key={u} className="text-center text-xs text-muted-foreground">
          {u}
        </div>
      ))}
      {skills.map((s, row) => (
...
export default function Dashboard() {
  ...
  const [overview, setOverview] = useState<
    null | import("@/lib/apiClient").DashboardOverview
  >(null);
  ...
  const totals = useMemo(() => {
    ...
    const baseAttrition =
      (overview?.metrics?.average_attrition_risk ?? 0.072) * 100;
    ...
    return {
      ...
      readiness: Math.max(
        0,
        Math.min(100, baseReadiness - shock * 0.6 + wave * 2),
      ),
      attrition: Math.max(
        0,
        baseAttrition + shock * 0.3 + Math.cos(tick / 2) * 0.8,
      ),
      training: Math.max(
        0,
        Math.min(100, baseTraining - shock * 0.4 + wave * 1.5),
      ),
    };
  }, [tick, shock, overview]);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex-row items-center justify-between">
            <CardTitle className="text-base font-medium flex items-center gap-2">
              <Users className="h-4 w-4 text-primary" /> {t("kpiPersonnel")}
            </CardTitle>
            <Badge variant="secondary">Live</Badge>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-semibold">
              {totals.personnel.toLocaleString()}
            </div>
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between">
            <CardTitle className="text-base font-medium flex items-center gap-2">
              <Activity className="h-4 w-4 text-primary" /> {t("kpiReadiness")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-baseline gap-2">
              <div className="text-3xl font-semibold">
                {totals.readiness.toFixed(0)}%
              </div>
          ...
          <CardHeader className="flex-row items-center justify-between">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  Higher intensity indicates shortage.
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="text-sm text-muted-foreground">
              Impact on readiness: {Math.round(-shock * 0.6)}%
            </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-start justify-between rounded-md border p-3">
              <div>
                <div className="text-sm font-medium">
                  Medical clearance lapse
                </div>
                <div className="text-xs text-muted-foreground">
                  3 personnel require immediate attention.
                </div>
              </div>
            ...
            <div className="flex items-start justify-between rounded-md border p-3">
              <div>
                <div className="text-sm font-medium">
                  Upcoming mission readiness check
                </div>
                <div className="text-xs text-muted-foreground">
                  Unit Charlie in 5 days.
                </div>
              </div>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>
              Predicted skill shortage in Cyber for Units B and D within next
              quarter. Recommend reskilling 12 personnel and prioritizing
              training module CYB-201.
            </p>
            <p>
              Attrition risk elevated for support staff due to relocation
              cycles. Consider targeted retention bonuses and flexible postings.
            </p>
          </CardContent>
        </Card>
      </div>
